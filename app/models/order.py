...
class Order(Base):
    ...
    amount_before_discount = Column(Numeric(10, 2), nullable=False, default=0)
    amount_total = Column(Numeric(10, 2), nullable=False, default=0)
    stripe_checkout_session_id = Column(String, nullable=True)
    stripe_payment_intent_id = Column(String, nullable=True)
    stripe_charge_id = Column(String, nullable=True)
    hubrise_order_id = Column(String, nullable=True)
    hubrise_raw_status = Column(String, nullable=True)
    hubrise_sync_status = Column(String, nullable=True)
    hubrise_last_error = Column(String, nullable=True)
    hubrise_synced_at = Column(DateTime(timezone=True), nullable=True)
    ...
    hubrise_logs = relationship("HubriseOrderLog", cascade="all, delete-orphan")
